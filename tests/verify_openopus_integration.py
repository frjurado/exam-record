...
async def verify_flow():
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        ...
        res = await client.get(
            f"/api/works/search?q=Jupiter&source=openopus&composer_id={openopus_id}"
        )
        if res.status_code != 200:
        ...
        if len(works) > 0:
            ...
            print(
                "WARNING: No works found, but API call succeeded. (Maybe OpenOpus is down or query is too vague?)"
            )
